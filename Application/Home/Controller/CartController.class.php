...
class CartController extends Controller
{
	...
	public function addCart()
	{
		...
		if ($info) {
			...
			//if cannot get info(goods is not already added in the cart), then run the below code:
			$goodsInfo = M('goods', '', 'db')->find($goodsId);

			if (isset($_POST['quantity'])) {
				//for addbag in goodsdetail screen
				$res = M('cart', '', 'db')->add(array('uid' => $uid, 'goodsId' => $goodsId, 'quantity' => $_POST['quantity'], 'price' => $goodsInfo['price'], 'name' => $goodsInfo['name']));
               // addbag in fav screen
			} else {
	...
	public function paypalCapture()
	{
		...
		if ($response['status'] == 'COMPLETED') {
			...
			$orderId = M('order')->where($where)->field('id')->find()['id'];
