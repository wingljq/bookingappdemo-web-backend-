...
class CartController extends Controller
{
	...
	public function checkout()
	{
		...
		if ($checkoutSession) {
			...
			$where['status'] = 0;
			$data = array();
			$data['out_trade_no'] = $checkoutSession->id;
			...
			if (isset($_POST['coupon'])) {
				if ($_POST['coupon']) {
					if ($user_coupon) {
						...
						$where['status'] = 0;
						$data = array();
						$data['orderId'] = $checkoutSession->id;
			...
			echo json_encode(array('status'=>'1','data'=>$checkoutUrl));
			// echo json_encode(array('paymentIntent' => $paymentIntent->client_secret, 'ephemeralKey' => $ephemeralKey->secret, 'customer' => $stuid,'data'=>$checkoutUrl));
	...
	public function paypalCapture()
	{
		...
		if ($response['status'] == 'COMPLETED') {
            ...
            if(C("paid_send_email") && is_file($this->view->parseTemplate("Common/order_paid_email.tpl"))){
                $out_trade_no = $outTradeNo;
                $pcset = M('pcset')->where('')->select()[0];
                $order = M('order')->where(array('uid' => $uid, 'payStatus' => 1, 'out_trade_no' => $outTradeNo))->find();
                $cartItems = M('cart')->where(array('uid' => $uid, 'status' => 2, 'out_trade_no' => $outTradeNo))->select();
                foreach($cartItems as $key => $cartItem){
                    $cartItems[$key]['total_price'] = $cartItem['price'] * intval($cartItem['quantity']);
                }
                $this->assign("order", $order);
                $this->assign("cartItems", $cartItems);
                $this->assign("siteUrl", rtrim(C("url")));
                $this->assign("pcset", $pcset);
                $sendHtml = $this->fetch("Common/order_paid_email.tpl");
                $subject = $pcset['companyName1'] . " has received your order!";
                $user = M('user')->where(array('uid' => $uid))->field('email')->find();
