...
function  showCalen(begCalenDate,orderDataJson){
	$(function(){
	
	
		
		var get = begCalenDate;
	...
	//	    },
			...
			monthNamesShort:  ["January", "February", "March", "April", "may", "June", "July", "August", "September", "October", "November", "December"] , //月份简写
			dayNames: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"] ,    	//周全称
			dayNamesShort: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],  //周简写
			noEventsMessage : "当月无数据",   							//listview视图下，无数据提示
			allDayText: "全天",											//自定义全天视图的名称
			allDaySlot: false,  										//是否在周日历上方显示全天
		// allDayDefault: true,										//是否为全天日程事件，显示这一天中所做的事情
			...
			firstDay: 0,   												//一周中显示的第一天是哪天，周日是0，周一是1，类推
			...
			height: 700,												//设置日历的高度，包括header日历头部，默认未设置，高度根据aspectRatio值自适应。
	...
	//      	console.log('Clicked on: ' + date.format());
			...
			eventClick: function(event, jsEvent) {	
				...
				var isAgree=event.isAgree;
	
				if(isAgree==1){
					return false;
				}
	
				layer.confirm(' Are you sure you want to agree? ', function(index) {
					$.ajax({
						type: 'get',
						url: "/insmile/index.php/Admin/Order/agree.html?orderId=" + orderId,
						dataType: 'json',
						success: function(data) {
							layer.msg(data.msg);
							if (data.status == 111) {
